...
2025‑07‑06 2nd pass
    • Restored *big* page header at the top.
    • Sidebar toggle renamed to “Show status (badge + score)” and now
      hides/ shows **both** the level badge *and* the affinity pill.
    • Threshold widgets already use `st.number_input` (direct typing).
      Reminder: a higher number = 더 어려운 레벨‑업 (누적 점수 기준)
"""
...
st.markdown(
    ...
    /* ---- Chat‑input tweaks (transparent button icon) */
...
with st.sidebar:
    ...
    st.markdown("### Score thresholds (cumulative points)")
    ...
    st.caption("※ 큰 숫자일수록 레벨‑업이 어려워집니다 (누적 점수 기준)")
